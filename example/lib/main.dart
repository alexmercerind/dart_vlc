...
class _DartVLCState extends State<DartVLC> {
  ...
  late Future<Player> initializePlayer;
  ...
  void didChangeDependencies() async {
    ...
    if (this.mounted) {
      ...
      this.setState(() {});
      Record record = await Record.create(id: 205, media: await Media.network(Uri.parse('http://22533.live.streamtheworld.com/LOS40.mp3')), pathFile: 'C:\\record_${DateTime.now().millisecondsSinceEpoch}.mp3');
      record.start();
  ...
  Future<Player> _initializePlayer() async {
    final _player = await Player.create(
      id: 0,
      videoWidth: 960,
      videoHeight: 640,
    );
      ...
      home: Scaffold(
        ...
        body: FutureBuilder<Player>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              if (player == null) player = snapshot.data;
              return ListView(
                shrinkWrap: true,
